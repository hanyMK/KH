...
<body>
	...
	<%@ include file="../common/menubar.jsp" %>
		...
		<div id="wrap_2">
			<div id="div_1">
				<div id="div_5">
					<button type="button" id="messageBtn"  data-bs-toggle="modal" data-bs-target="#writeMessage" data-bs-whatever="@mdo" data-bs-target="#writeMessage" >쪽지쓰기</button><br><br>
					<a class="aMsg" href="<%=contextPath%>/message.me?msgpage=1">받은 쪽지함</a><br><br>
					<a class="aMsg" href="<%=contextPath%>/messageSand.me?msgSpage=1">보낸 쪽지함</a>
				</div>
			</div>
			<div id="div_2">
				<br>
				<hr>
					<h2 id="pMName">받은 쪽지함</h2>
				<hr>
            		<br><br>
				<table class="tableMessage" border="1" id="tableMessage1">
					<thead>
						<tr>
							<th width="50" id="thTitle"></th>
							<th width="150" id="thTitle">보낸사람</th>
							<th width="300" id="thTitle">제목</th> 
							<th width="150" id="thTitle">날짜</th>
							<th width="60" id="thTitle"></th>
							<th width="90" id="thTitle">상태</th>
						</tr>
					</thead>
					<tbody>
						<% if(list.isEmpty()) {%>
						...
						<% } else { %>
							...
							<tr>
								...
								<td><%=msg.getFromNic() %></td>
								<td><%=msg.getMessageTitle() %></td>
								<td><%=msg.getMessageDate() %></td>
								<td><input type="hidden" id="messageAlert" value="<%=msg.getMessageAlertStatus()%>"></td>
								<td>
									<% if(msg.getMessageAlertStatus().equals("Y")){ %>
										<input type="text" class="checkMsgg"  value="읽음" readonly>
									<% } else if(msg.getMessageAlertStatus().equals("N")){ %>
										<input type="text" class="checkMsgg" value="안읽음" readonly>
									<% } %>
								</td>
							</tr>
							<% } %>
	            ...
	            <div align="center" class="paging-area">
		        	...
		        	<% if(currentPage != 1) {%>
		               <button onclick="location.href='<%= contextPath %>/message.me?msgpage=<%= currentPage - 1 %>'" class="actBtn">&lt;</button>
		         	<% } %>
		
					<% for(int i = startPage; i <= endPage; i++) {%>
						<% if(currentPage != i){ %>
							<button onclick="location.href='<%=contextPath %>/message.me?msgpage=<%=i %>'" class="actBtn"><%=i %></button>
						<%} else { %>
	...
	<script>
			...
			$(function(){
	    		$('.tableMessage>tbody>tr').click(function(){
	    			location.href = '<%=contextPath%>/enrollDetail.ms?msgdno=' + $(this).children().children().eq(0).val();
	    		});
	    	});
