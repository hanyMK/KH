...
request.setCharacterEncoding("UTF-8");
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			String savePath = application.getRealPath("/resources/board/review");
			...
			int result = new ReviewService().updateReviewBoard(oB, oAList);
			
			if(result > 0) {
				request.getSession().setAttribute("alertMsg", "게시글 수정이 성공했어요!");
				response.sendRedirect(request.getContextPath() + "/detailView.re?opage=" + boardNo);
			} else {
