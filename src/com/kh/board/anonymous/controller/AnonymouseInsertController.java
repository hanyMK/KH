...
/**
 * Servlet implementation class AnonymouseInsertController
 */
@WebServlet("/insert.an")
public class AnonymouseInsertController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AnonymouseInsertController() {
        super();
...
//	}
		...
		if(ServletFileUpload.isMultipartContent(request)) {		// 만약 from태그에서 가져와서 보내준파일... ~~
			...
			if(!multiRequest.getParameter("voteTitle").equals("")) {
				...
				int memberNo = Integer.parseInt(multiRequest.getParameter("userNo"));	//작성자 번호(식별기준)
				...
				a.setBoardTitle(boardTitle);
				a.setBoardContent(boardContent);
				...
				//해시태그 존재 유무판단
				int result = as.insertAnonymousWithVote(a, list, cvt, queryList); // 투표가 있을때의 데이터 가공 완료 후 서비스 전달
	...
	//------------해시태그가 존재한다면 insert-----------------------
				...
				//System.out.println("마지막 여기까지 오나?? 값이 담긴 clList");
				...
				if(cvt.getVoteTitle() != null && !cvt.getVoteTitle().equals("")) { // 투표, 투표항목 생성자에 값이 비어있으므로
					a.setVoteYN("Y"); // 투표가 있을때 Y값
				} else {
					a.setVoteYN("N"); // 투표가 없을때 N값
				}
				...
				int result = as.insertAnonymous(a, list);// 투표가 없을때의 데이터 가공처리 후 서비스로 전달
